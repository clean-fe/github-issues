import { compose } from "../constants/compose.js";
import { fetchJSON } from "../constants/fetch.js";
import { filter } from "../constants/filter.js";
import { Result } from "../constants/result.js";

export const renderIssuesByStatus = (status = "open", ul, url) => {
    if (!ul) {
        throw new Error("ul must be a DOM element");
    }
    if (typeof status !== "string") {
        throw new Error("status must be a string");
    }
    if (typeof url !== "string") {
        throw new Error("initialArgs must be an object");
    }
    compose(
        fetchJSON,
...
export const renderIssueList = (node) => {
    if (!node) {
        throw new Error("node must be a DOM element");
    }
    return data => {
        const issueListHtmlResult = createIssueListHtml(data);
        if (issueListHtmlResult.ok) {
            node.innerHTML = issueListHtmlResult.value;
        }
        return issueListHtmlResult;
    }
}
